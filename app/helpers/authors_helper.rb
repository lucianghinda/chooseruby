...
module AuthorsHelper
  # Returns an array of social link hashes for the author
  # Each hash contains :url, :name, :icon_path (SVG path data)
  # Only returns links where the URL is present
  def author_social_links(author)
    ...
    if author.gitlab_url.present?
      links << {
        url: author.gitlab_url,
        name: "GitLab",
        icon_path: "M23.955 13.587l-1.342-4.135-2.664-8.189c-.135-.423-.73-.423-.867 0L16.418 9.45H7.582L4.918 1.263c-.135-.423-.73-.423-.867 0L1.386 9.45.044 13.587c-.1.31.006.648.27.856l11.686 8.546 11.686-8.546c.264-.208.37-.546.27-.856z"
      }
    end

    if author.twitter_url.present?
      links << {
    ...
    if author.bluesky_url.present?
      links << {
        url: author.bluesky_url,
        name: "Bluesky",
    ...
    if author.twitch_url.present?
      links << {
        url: author.twitch_url,
        name: "Twitch",
        icon_path: "M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714Z"
      }
    end

    if author.ruby_social_url.present?
      links << {
        url: author.ruby_social_url,
        name: "Ruby.social",
